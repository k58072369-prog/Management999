...
import {
  useGetFinanceSummary, useListInvoices, useListExpenses, useUpdateInvoice,
  getListInvoicesQueryKey, getGetFinanceSummaryQueryKey,
...
import { Plus, Wallet, TrendingUp, TrendingDown, FileText, Receipt, Search } from "lucide-react";
...
export default function Finance() {
  ...
  const filteredInvoices = invoices?.filter(inv =>
    !invoiceSearch || inv.student_name?.toLowerCase().includes(invoiceSearch.toLowerCase())
  );

  const markPaid = (invoice: Invoice) => {
    updateInvoice.mutate({ id: invoice.id, data: { status: "مدفوع" } as any }, {
      onSuccess: () => {
